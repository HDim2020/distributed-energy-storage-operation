...
@Builder
@Getter
public class IeSocketThresholdSettings implements IInformationElement {
    /**
     * 电压 2字节单位V
     */
    private int voltage;
     ...
     * @return 实体对象
     */
    public static IeSocketThresholdSettings decode(byte[] buf) {
        if (buf.length < 34) {
            return null;
        }
        //以下的状态位为1时会导致设定值和返回值不一致，先屏蔽掉
        //BIT3:过压状态改为0
        buf[13] &= 0xF7;
        //BIT3:欠压状态改为0
        buf[18] &= 0xF7;
        ...
        //BIT3:超温状态改为0
        buf[28] &= 0xF7;
        IeSocketThresholdSettingsBuilder builder = IeSocketThresholdSettings.builder()
