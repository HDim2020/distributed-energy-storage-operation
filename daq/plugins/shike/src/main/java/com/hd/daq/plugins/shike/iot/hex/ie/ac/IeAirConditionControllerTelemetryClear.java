...
/**
 * 大功率空调控制器重置电量信息体元素（设备上行）
 *
 * @author ymm
 */
@Builder
@Getter
public class IeAirConditionControllerTelemetryClear implements IInformationElement {
    /**
     * 电压（2字节单位V）
     */
    private int voltage;
     ...
     * @return 实体对象
     */
    public static IeAirConditionControllerTelemetryClear decode(byte[] buf) {
        if (buf.length != 18) {
            return null;
        }
        return IeAirConditionControllerTelemetryClear.builder()
