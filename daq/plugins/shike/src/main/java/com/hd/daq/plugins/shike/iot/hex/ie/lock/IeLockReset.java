...
/**
 * 办公锁设置恢复出厂设置响应信息体元素（设备上行）
 *
 * @author ymm
 */
@Builder
@Getter
public class IeLockReset implements IInformationElement {
    /**
     * 设备类型 1字节
     */
     ...
     * @return 实体对象
     */
    public static IeLockReset decode(byte[] buf) {
        if (buf.length < 7) {
            return null;
        }
        return IeLockReset.builder()
    ...
    public int getCmdCode() {
        return DeviceCmdType.CMD_0F.getId();
    }
