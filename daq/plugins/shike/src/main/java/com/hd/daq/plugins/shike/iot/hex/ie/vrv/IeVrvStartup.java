...
@Builder
@Data
public class IeVrvStartup implements IInformationElement {
    /**
     * 设备类型码 1字节
     */
     ...
     * @return 实体对象
     */
    public static IeVrvStartup decode(byte[] buf) {
        if (buf.length < 15) {
            return null;
        }
        return IeVrvStartup.builder()
                .deviceTypeCode(Byte.toUnsignedInt(buf[0]))
                .checkSum((Byte.toUnsignedInt(buf[2]) << 8) | Byte.toUnsignedInt(buf[3]))
                .version(new String(buf, 5, 8, StandardCharsets.US_ASCII))
                .build();
    }
