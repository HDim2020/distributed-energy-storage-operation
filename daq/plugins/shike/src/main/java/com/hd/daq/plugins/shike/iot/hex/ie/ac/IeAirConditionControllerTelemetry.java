...
/**
 * 大功率空调控制器遥测数据信息体元素（设备上行）
 *
 * @author ymm
 */
@Builder
@Data
public class IeAirConditionControllerTelemetry implements IInformationElement {
    /**
     * 电压（2字节单位V）
     */
    private int voltage;
     ...
     * @return 实体对象
     */
    public static IeAirConditionControllerTelemetry decode(byte[] buf) {
        if (buf.length != 18) {
            return null;
        }
        return IeAirConditionControllerTelemetry.builder()
